...
let lastReadOtherTextKey = null;       // 用户最后阅读的其他文本的contentKey（用于红点提示）
...
function clearOtherTextHistory() {
    ...
    lastReadOtherTextKey = null;
}

/**
 * 检查是否有未读的其他文本
 * @returns {boolean} 如果有未读内容返回 true
 */
function hasUnreadOtherText() {
    // 如果缓存为空，没有未读内容
    if (otherTextHistoryCache.length === 0) {
        return false;
    }
    // 获取最后一条文本的 contentKey
    const lastItem = otherTextHistoryCache[otherTextHistoryCache.length - 1];
    const lastKey = lastItem ? lastItem.contentKey : null;
    // 如果从未阅读过，或者最后一条与已读记录不同，则有未读内容
    return lastKey !== lastReadOtherTextKey;
}
...
function enterOtherTextHistoryMode() {
    ...
    // 更新已阅读标记：记录当前最后一条文本为已阅读
    if (otherTextHistoryCache.length > 0) {
        const lastItem = otherTextHistoryCache[otherTextHistoryCache.length - 1];
        lastReadOtherTextKey = lastItem ? lastItem.contentKey : null;
    }
...
function updateTextHistoryTabState(type, active) {
    ...
    if (textHistoryTab) {
        if (active) {
            ...
            // 进入其他文本回溯模式时，移除红点提示
            if (type === 'other') {
                const redDot = textHistoryTab.querySelector('.unread-dot');
                if (redDot) {
                    redDot.remove();
                }
                textHistoryTab.classList.remove('has-unread');
            }
        } else {
...
function createPanelTabsBar(tabs) {
    ...
    if (!isSubPanelMode) {
        ...
        // 检查是否有未读内容，显示红点提示
        if (hasUnreadOtherText()) {
            const redDot = document.createElement('span');
            redDot.className = 'unread-dot';
            otherTextHistoryBtn.appendChild(redDot);
            otherTextHistoryBtn.classList.add('has-unread');
        }
