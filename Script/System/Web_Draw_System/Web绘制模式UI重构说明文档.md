...
- **选项卡组成**：
  ...
  3. **文本回溯选项卡容器**：位于选项卡栏最右侧（使用 `margin-left: auto` 右对齐），包含以下两个按钮紧挨排列：
     - **指令文本回溯选项卡**：id 为 `__text_history__`，用于查看指令行动描述和结算文本
     - **其他文本回溯选项卡**：id 为 `__other_text_history__`，用于查看其他游戏文本
...
#### 2.2.1.2 文本回溯功能（2026-03-05新增，2026-03-09拆分为双缓存）

文本回溯是一个**纯前端功能**，允许玩家查看游戏过程中显示过的历史文本记录。**从2026-03-09起，文本回溯拆分为两个独立功能：指令文本回溯和其他文本回溯。**

##### 指令文本回溯

- **功能说明**：
  - 点击"指令文本回溯"选项卡后，主界面被指令文本回溯面板覆盖
  - 显示最近200条指令相关文本记录，保留原有的字体样式和颜色
  - 文本按时间顺序排列，最新的文本在底部
  - 点击关闭按钮或其他选项卡可退出指令文本回溯模式

- **缓存机制**：
  - 前端维护一个最大长度为200的指令文本历史缓存数组 `textHistoryCache`
  - 前端维护一个内容去重集合 `textHistoryContentSet`，用于基于内容的去重
  - **缓存来源**（重要）：
    - 指令行动描述文本：由后端 `talk.py` 的 `handle_talk_draw()` 函数统一收集
    - 指令行动结算文本：由后端 `settle_behavior.py` 的 `handle_settle_behavior()` 收集
    - 前端通过 `state.instruct_texts` 一次性接收所有文本
  - 缓存支持的类型：
    - 结算文本：`settlement` - 指令行动结算时的完整输出文本（包括行为描述和数值变化）
    - 行为描述文本：`description` - 保留类型用于兼容，与 `settlement` 使用相同的富文本解析

- **相关后端字段**：
  - `cache.web_instruct_texts` - 指令文本列表

##### 其他文本回溯

- **功能说明**：
  - 点击"其他文本回溯"选项卡后，主界面被其他文本回溯面板覆盖
  - 显示最近200条其他游戏文本记录，保留原有的字体样式和颜色
  - 文本按时间顺序排列，最新的文本在底部
  - 点击关闭按钮或其他选项卡可退出其他文本回溯模式

- **红点提示功能**（2026-03-09新增）：
  - 当有新的其他文本未被玩家阅读时，在"其他文本回溯"按钮右上角显示红点提示
  - 判断逻辑：
    - 前端维护变量 `lastReadOtherTextKey` 记录玩家最后阅读的文本的 contentKey
    - 当缓存最后一条文本的 contentKey 与 `lastReadOtherTextKey` 不同时，显示红点
    - 当缓存为空时，不显示红点
  - 消除红点：玩家点击进入"其他文本回溯"界面后，自动将当前最后一条文本标记为已阅读
  - 相关函数：`hasUnreadOtherText()` 检查是否有未读内容
  - 相关CSS类：`.has-unread`（按钮状态）、`.unread-dot`（红点元素）

- **上次阅读位置标记功能**（2026-03-09新增）：
  - 进入"其他文本回溯"面板时，如果满足以下条件，会显示"上次阅读位置"标记：
    1. 玩家上次阅读的最后一条文本不是当前记录的最后一条文本
    2. 上次阅读的文本仍然存在于缓存记录中（未被移除）
    3. 页面内容超出一屏，出现滚动条
  - 标记显示：在上次阅读的最后一条文本下方插入橙色分隔线，中间显示"上次阅读位置"文字
  - 自动滚动：满足条件时，页面自动滚动到标记位置居中显示
  - 不满足条件时，默认滚动到底部显示最新文本
  - 相关CSS类：`.text-history-last-read-marker`、`.marker-line`、`.marker-text`

- **缓存机制**：
  - 前端维护一个最大长度为200的其他文本历史缓存数组 `otherTextHistoryCache`
  - 前端维护一个内容去重集合 `otherTextHistoryContentSet`，用于基于内容的去重
  - **缓存来源**（重要）：
    - **行为循环期间的其他文本**：由后端 `io_web.append_current_draw_element()` 函数在行为循环期间自动收集
...
- **相关后端字段**：
  - `cache.web_other_texts` - 其他文本列表

##### 通用功能

- **去重机制**：
  - 两个缓存各自使用 Set 集合存储已添加内容的 `type:text` 组合
...
- **结算文本富文本解析**：
  ...
  - `settlement`、`description` 和 `other` 类型都使用此函数进行解析
...
- **辅助函数**：
  - `addToTextHistory(item)` - 添加指令文本元素到历史缓存，支持内容去重
  - `addToOtherTextHistory(item)` - 添加其他文本元素到历史缓存，支持内容去重
  - `addValueChangesToTextHistory(valueChanges, source)` - 添加数值变化文本到历史缓存（保留用于兼容）

- **界面结构**：
  - 头部区域：标题（"指令文本回溯"或"其他文本回溯"）+ 记录条数 + 关闭按钮
...
- **相关代码路径**：
  - 缓存变量：`static/game.js` 中的 `textHistoryCache`、`otherTextHistoryCache`、`isTextHistoryMode`、`isOtherTextHistoryMode`
  - 缓存函数：`static/game.js` 中的 `addToTextHistory()`、`addToOtherTextHistory()`
  - 状态管理：`static/game.js` 中的 `enterTextHistoryMode()`、`exitTextHistoryMode()`、`enterOtherTextHistoryMode()`、`exitOtherTextHistoryMode()`
  - 界面渲染：`static/js/new_ui_components.js` 中的 `createTextHistoryPanel(type)`、`createTextHistoryElement()`
  - 样式定义：`static/css/style.css` 中的 `.text-history-*` 类（含 `.text-history-settlement`、`.text-history-other`）
...
- **头像图片**：
  - 优先使用 `{角色名}_头部.png` 作为头像图片
  - 如果没有现成的头部图片文件，则从全身图中动态截取
  - **截取逻辑**：正方形头像，下边中点=鼻子向下偏移图像高度4%，上边中点=鼻子向上偏移图像高度6%
  - 鼻子位置从角色的 `{角色名}_body.json` 文件中读取
  - 截取后的头像会被缓存，避免重复处理
- **布局规则**：单行显示，最多显示5个头像
- **分页功能**：
  - 当场景内角色超过5人时，在区域底部显示分页按钮行（绝对定位，下对齐居中）
  - **上一页按钮**：切换到上5个角色，在第一页时会循环到最后一页
  - **下一页按钮**：切换到下5个角色，在最后一页时会循环到第一页
  - **显示全部按钮**：打开"场景内全部角色"弹出面板
- **全部角色面板**：
  - **触发方式**：点击"显示全部"按钮
  - **显示内容**：场景内全部角色（包括玩家和当前交互对象）
  - **布局规则**：每行显示10个角色，最多显示6行（共60个角色可见）
  - **滚动功能**：当角色超过60人时，使用右侧滚动条拖动显示区域
  - **角色样式**：与头像区相同的样式（包含头像图片）
  - **交互**：点击角色可切换交互对象并自动关闭面板；点击面板外部可关闭面板
- **交互**：点击头像可切换当前交互对象
...
- **角色立绘高度缩放规则**：
  - 测量可用高度时，先临时将图片高度设为1px，避免图片naturalHeight影响flex布局
  - 可用高度基于主场景区域（`.new-ui-main-scene`）高度计算
...
- **大类（7个）**：嘴(mouth)、手(hand)、性爱(sex)、阴茎(penis)、道具(tool)、技艺(arts)、其他(other)
  - **注意**：阴茎(penis)大类仅在H模式中显示，非H模式下不显示该选项
- **初始状态**：进入主界面时，不会默认选择任何交互类型，需要用户手动点击或悬浮选择
...
##### 2.2.6.1 悬浮预览模式（2026-03-04新增）
支持鼠标悬浮自动预览大类的小类列表，提供更流畅的交互体验：

- **悬浮展开**：当鼠标移动到某个大类按钮上时，会临时展开该大类的小类列表，并隐藏其他大类按钮
- **状态保持**：一旦进入悬浮预览状态，会保持当前展开的大类，直到鼠标完全离开整个交互类型面板区域才收起
  - 这样设计是为了避免因布局变化导致的循环进入/退出问题（如：展开后按钮位置改变，鼠标仍在原位置导致反复触发）
  - 在面板内部移动鼠标时，可以直接切换到其他大类的预览，无需先收起当前大类
- **悬浮收起**：当鼠标移出整个交互类型面板区域时，如果没有点击选中任何大类，则恢复到初始状态（显示所有大类，不展开任何小类）
- **布局实现**：
  - 隐藏其他大类时使用 `visibility: hidden` 而非 `display: none`，确保元素仍占据空间，面板大小不变，避免鼠标位置突然处于面板外部而触发收起
  - 大类卡片和小类列表使用绝对定位（`position: absolute`），每个卡片位置固定，不受其他卡片隐藏影响
  - 整组大类卡片在容器中垂直居中排列：
    - 计算所有大类卡片的总高度：`totalHeight = count × 5.3125rem - 0.9375rem`
    - 计算居中偏移：`centerOffset = (容器高度 47.8125rem - 总高度) / 2`
    - 每个大类卡片位置：`top = centerOffset + index × 5.3125rem`（5.3125rem = 卡片高度 4.375rem + 间距 0.9375rem）
  - 小类列表位置在对应大类卡片下方，继承相同的居中偏移，同样使用绝对定位
    - 初始渲染时：从大类卡片的计算位置（`topPosition`）加上卡片高度和间距
    - 悬浮预览时：从目标卡片的 `style.top` 读取位置，加上卡片高度和间距
    - 点击选中后：从激活卡片的 `style.top` 读取位置，加上卡片高度和间距
    - 计算公式统一为：`minorTop = majorCardTop + 4.375rem + 0.9375rem`
- **与点击模式的区别**：
  - 悬浮模式是纯前端的临时预览效果，不会修改后端状态
  - 点击模式会通知后端确认选择
- **优先级**：当已有点击选中的大类时，悬浮预览模式不生效，保持点击选中的状态

##### 2.2.6.2 交互流程
1. **悬浮探索**：鼠标悬浮到大类卡片上 → 临时展开该大类的小类列表（预览模式），在面板内移动可切换预览不同大类
2. **点击确认**：点击大类卡片或小类卡片 → 确认选择（进入聚焦模式）
3. **执行指令**：点击高亮的身体部位或浮现指令 → 执行对应指令
4. **清空选择**：点击空白区域 → 重置所有选择，恢复初始状态

#### 2.2.7 交互对象信息区（右侧）
- **位置**：主画面区的右侧边缘，使用绝对定位（不影响角色立绘的居中）
- **内容**：当前交互对象的核心状态信息
- **排版规则**：
  - **第一行**：角色名字 + 好感度 + 信赖度
  - **第二行**：体力槽 + 气力槽
...
#### 2.2.8 对话框区域（底部）
- **位置**：画面最下方，使用绝对定位覆盖在主内容之上（`position: absolute; bottom: 0`）
- **样式**：经典文字冒险游戏/恋爱游戏的对话框风格（半透明背景，左侧角色名字，内部台词）
- **交互**：左键点击推进文本，按住右键或Ctrl键快速跳过
- **布局特性**：对话框不参与flex布局，显示/隐藏时不会影响角色立绘区域的大小
...
**交互状态恢复机制**:
- 主界面刷新后，后端会携带当前的交互选择状态（`current_major_type` 和 `current_minor_type`）
- 前端在渲染交互类型面板时，如果检测到有已选中的小类，会自动触发 `selectMinorType()` 请求
- 这确保了刷新后的交互区域能正确显示无部位指令并进入聚焦模式
...
### 5.6 角色立绘高度计算（与裁切功能的兼容）

由于裁切功能会改变图片的实际像素尺寸，需要特殊处理才能正确计算角色立绘的显示高度。
...
#### 5.6.2 解决方案
在测量可用高度之前，临时将图片高度设为 `1px`：
1. 保存图片原始高度
2. 将图片高度设为 `1px`，消除对布局的影响
3. 使用 `void mainScene.offsetHeight` 强制浏览器同步重新计算布局
4. 测量 `.new-ui-main-scene` 的实际可用高度
5. 计算目标高度：`min(1024, 可用高度 - 20)`
6. 设置图片高度为目标高度

#### 5.6.3 技术要点
- **同步布局计算**：读取 `offsetHeight` 会强制浏览器完成所有待处理的布局计算
- **边距预留**：可用高度减去 20px 作为上下边距
- **失败恢复**：如果计算结果无效，恢复图片原始高度
...
#### 6.3.1 交互对象名字显示机制
- **问题场景**：当切换到某角色并显示其行为文本时，需要显示该角色当时的交互对象名字
- **潜在问题1**：角色指令结算完毕后可能已更换交互对象，实时读取会导致显示错误
- **潜在问题2**：当交互对象是玩家（cid=0）时，简单的 `if target_character_id` 判断会将 0 视为 False
- **解决方案**：
  1. 在记录非当前交互对象的行为文本时，同时保存其当时的交互对象ID
  2. 使用 `is not None` 判断而非简单的布尔判断，确保玩家（cid=0）作为交互对象时能正确保留
- **数据流程**：
  1. `talk.py` 调用 `add_dialog_text()` 时传递 `target_character_id`（使用 `is not None` 判断）
  2. `dialog_box.py` 在 `minor_entry` 中保存 `target_character_id`
  3. `web_server.py` 的 `handle_switch_target()` 使用保存的 `target_character_id` 而非被切换到的角色ID
- **效果**：对话框显示格式为 `角色名 → 交互对象名`，交互对象名为该角色对话时的真实交互对象（包括玩家）
